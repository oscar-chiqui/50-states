    ...
    methods: {
        fetchStateData() {
            this.$stateService.getOneState(this.state.name).then( state => {
                this.state = state
                this.dataReady = true
            }).catch( err => { // 404 not found
                if ( err.response && err.response.status === 404 ){
                    this.state.name = '?'

                }else { // 500 server error 
                    alert('Sorry, error fetching data about this state') // Message to the client
        ...
        setMapView() {
            if (this.mapReady && this.dataReady ){  // Shows the correct part of world and zoom level//
